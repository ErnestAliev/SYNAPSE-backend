...
function createAiRouter(deps) {
  ...
  function buildWebSearchEntitySummaryInstructions(entityType = 'shape') {
    if (entityType === 'person') {
      return [
        'Нужен не пересказ биографии, а глубокий портрет человека для стратегической работы.',
        'Строй сводку как досье на персону с короткими подзаголовками и абзацами.',
        'Обязательно раскрой: кто это и какой у него реальный вес; биография и карьерная траектория; где и с кем светился; публичные и деловые связи; кто его круг общения и аффилированные лица; сильные стороны, слабые стороны, уязвимости, мотивация, стиль поведения, ментальность, страна происхождения и что это обычно означает в переговорах или взаимодействии.',
        'Если есть дата рождения, возраст, место рождения или иные биографические маркеры — включай. Если есть точная дата рождения, можно кратко указать знак зодиака и дать культурно-бытовую интерпретацию как отдельную мягкую аналитическую ремарку, не выдавая это за факт высокой достоверности.',
        'Если в источниках видны решения, высказывания, конфликты, партнерства, расследования, споры, паттерны поведения или повторяющиеся союзы, обязательно сделай вывод, что это говорит о человеке.',
        'Заверши блоком "Что это значит": 2-4 плотных предложения о том, как этот портрет можно использовать при построении стратегии, контакта, переговоров, атаки или защиты в контексте аналитического сервиса.',
      ];
    }
