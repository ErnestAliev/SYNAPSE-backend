...
function createAiRouter(deps) {
  ...
  function buildWebSearchEntitySummaryInstructions(entityType = 'shape') {
    if (entityType === 'person') {
      return [
        'Собери плотное стратегическое досье на человека только по веб-источникам. Не выдумывай факты. Не делай PR и не пересказывай биографию.',
        'Нужен не набор фактов, а понятный трек человека: кто он, откуда взялся, как рос, где находится сейчас, с кем связан, на что влияет, где его сильные и слабые стороны, и что это значит на практике.',
        'Строй ответ по блокам.',
        '1. Кто это — имя, страна, текущая роль, реальный вес.',
        '2. Краткий профиль — дата и место рождения, образование, специализация, ключевые города и страны.',
        '3. Трек по годам — роль, организация, что это дало к статусу и влиянию.',
        '4. Текущее положение — должность, с какого времени, текущие проекты, зоны контроля.',
        '5. Контур связей — отдельно семейный, деловой, институциональный.',
        '6. География и международный след.',
        '7. Последние события за 12–24 месяца.',
        '8. Репутационные сигналы, конфликты, суды, расследования, санкции, претензии.',
        '9. Сильные стороны.',
        '10. Слабые стороны и уязвимости.',
        '11. Поведенческий контур — только мягкие выводы по паттернам источников.',
        '12. Что это значит — как читать этого человека для стратегии, переговоров, контакта, атаки или защиты.',
        'Различай факт, заявление стороны, сообщение СМИ и аналитический вывод.',
        'Пиши кратко, плотно, тезисно, без воды.',
      ];
    }
      ...
      const response = await fetch('https://api.openai.com/v1/responses', {
        ...
        body: JSON.stringify({
          ...
          instructions: [
            ...
            'Сделай развернутую, полезную и беспристрастную сводку по запросу.',
            'Работай в режиме нейтрального журналистского досье, а не PR-описания.',
