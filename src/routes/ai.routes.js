...
function createAiRouter(deps) {
  ...
  const WEB_SEARCH_MAX_OUTPUT_TOKENS = 4200;
      ...
      const response = await fetch('https://api.openai.com/v1/responses', {
        ...
        body: JSON.stringify({
          ...
          instructions: [
            ...
            'Структурируй сводку как единый связный текст с короткими абзацами: кто это/что заявляет о себе, что пишут внешние источники, какие есть связи/риски/спорные сюжеты, и что особенно важно для дальнейшей проверки.',
            'Пиши достаточно подробно, чтобы текст был полезен для ручного переноса на канву.',
