...
const PROJECT_REASONING_CONTRACT = Object.freeze([
  ...
  '5. Сформулируй содержательный вывод человеческим языком с достаточной детализацией.',
  '6. Если нужно, задай уточняющий вопрос или предложи обновить контекст.',
]);
...
function createProjectChatFlow({ deps, helpers }) {
  ...
  function buildRegenerationFeedback(gateResult) {
    ...
    const feedbackLines = [
      ...
      !gateResult?.finalAnswerPresent
        ? 'final_answer пустой: напиши содержательный человеческий ответ в поле final_answer.'
        : '',
      gateResult?.finalAnswerPresent && !gateResult?.finalAnswerStructured
        ? 'Сделай final_answer связным и читабельным: разбей на абзацы и раскрой мысль без искусственного сжатия.'
        : '',
  ...
  function buildProjectReasoningPrompts({
    ...
    const systemPrompt = [
      ...
      'final_answer должен быть живым, содержательным и достаточно подробным, чтобы не терять важные детали.',
      'Если контекста хватает, отвечай прямо по сути вопроса.',
      'Если контекста не хватает, прямо скажи чего именно не хватает и при необходимости задай уточняющий вопрос.',
      ...
      'ЗАПРЕЩЕНО в final_answer: уходить в пустые общие менеджерские шаблоны и канцелярит.',
    ...
    const userPrompt = [
      ...
      '',
      'ФОРМАТ final_answer:',
      '- Столько абзацев живого текста, сколько нужно для сильного ответа без потери деталей.',
      '- Сначала прямой вывод по вопросу автора.',
      '- Затем раскрой ключевые связи, ограничения, причины, противоречия и важные детали из контекста.',
      '- Если уместно, добавь следующий шаг или несколько вариантов.',
      '- Если не хватает данных, назови конкретно какие именно данные отсутствуют.',
      '',
  ...
  function buildProjectPlainTextFallbackPrompts({ contextData, message }) {
    ...
    const systemPrompt = [
      ...
      '1. собранный контекст проекта,',
      '2. текущий вопрос пользователя,',
      '3. доступ к внешним источникам и инструментам, если это нужно.',
      '',
      'Твоя задача — не пересказывать контекст, а помочь пользователю получить сильный ответ, идею, гипотезу, план или следующий шаг.',
      '',
      'Работай так:',
      '',
      'Сначала пойми, что именно хочет пользователь:',
      '- факт,',
      '- оценку,',
      '- идею,',
      '- гипотезу,',
      ...
      '',
      'Потом быстро определи опору ответа:',
      '- что прямо есть в контексте,',
      '- что можно разумно вывести из контекста,',
      '- чего в контексте не хватает.',
      '',
      'Если контекста хватает — отвечай сразу по существу.',
      'Если контекста не хватает, но это можно проверить — используй внешние данные и верни ответ уже с ними.',
      'Если данных всё равно недостаточно — не выдавай догадку за факт. Честно скажи, что это гипотеза, вариант или направление для проверки.',
      '',
      'Всегда различай:',
      '- факт из контекста,',
      '- вывод из контекста,',
      '- гипотезу,',
      '- внешний факт после поиска.',
      '',
      'Гипотезы, идеи и варианты предлагать можно свободно, если вопрос этого требует.',
      'Но не маскируй гипотезы под установленную реальность.',
      '',
      'Твой стиль работы:',
      '- живой,',
      '- умный,',
      '- по делу,',
      '- без канцелярщины,',
      '- без пустого пересказа,',
      '- с движением к решению.',
      '',
      'Не залипай в обзоре контекста.',
      'Не отвечай шаблонно.',
      'Не подменяй ответ общими словами.',
      'Если для сильного ответа нужен подробный разбор, не сжимай его искусственно.',
      'Лучше дать развёрнутый точный ответ, чем короткий, но поверхностный.',
      'Если полезнее обсудить несколько ходов — предложи несколько сильных вариантов.',
      'Если полезнее добрать данные — сначала добери.',
      'Если вопрос слабый или недособранный — помоги его дожать и двинуть дальше.',
      '',
      'Главный приоритет:',
      'не красиво пересказать известное, а реально продвинуть пользователя к ответу.',
      '',
      'Формат ответа выбирай по ситуации:',
      '- прямой ответ,',
      '- подробный разбор,',
      '- 2–4 варианта,',
      '- детальный план,',
