...
function buildEntityAnalyzerBasePrompt({ entityType, allowedFields }) {
  return [
    ...
    'ПРАВИЛО ГРАНИЦ: Анализируй ТОЛЬКО текущую сущность. Если в тексте есть факты, описывающие ДРУГИЕ сущности (других людей, здания, компании, чужие проблемы) — КАТЕГОРИЧЕСКИ запрещено добавлять их в поля (теги, маркеры и т.д.) текущей сущности. Отправляй чужие факты в массив ignoredNoise. ИСКЛЮЧЕНИЕ: для shape description может кратко фиксировать связанный контекст одной или нескольких сущностей, если сам shape используется как свободный вспомогательный блок для этого контекста.',
...
function buildEntityTypeSpecificSections(entityType) {
  ...
  if (entityType === 'shape') {
    return [
      ...
      '- Shape может использоваться как контекст, заметка, мысль, пояснение, расшифровка, расширение описания, пояснение связи, дополнительный слой к событию, человеку, компании, ресурсу, проекту или любой группе узлов.',
      '- У shape нет одной фиксированной роли. Не своди его только к связи, только к контейнеру, только к заметке или только к структурному узлу.',
      '- Если пользователь дал длинный содержательный текст, description должен быть краткой выжимкой самого содержания этого текста.',
      '- Не трать description на объяснение, зачем существует shape. Описывай то, что внутри него зафиксировано.',
      '- Если shape привязан к одной сущности, он может быть просто дополнительным контекстом к ней. Если привязан к нескольким сущностям, он может раскрывать их общий контекст.',
      '',
